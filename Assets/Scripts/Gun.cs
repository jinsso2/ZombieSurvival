...
public class Gun : MonoBehaviour {
    ...
    // 발사 시도 (총이 발사 가능한 상태에서만 실행, shot 메서드를 안전하게 감싸는 껍데기)
    public void Fire() {
        // 현재 발사가 가능한 상태
    ...
    private void Shot() {
        ...
        if(Physics.Raycast(fireTransform.position, fireTransform.forward, out hit, fireDistance))
        {
            ...
            if(target != null)
            {
                // 상대방의 OnDamage 함수를 실행시켜 상대방에 총알의 공격력 만큼 데미지 주기
    ...
    private IEnumerator ShotEffect(Vector3 hitPosition) {
        ...
        // 총격 소리 재생
        // PlayOneShot() : 재생할 오디오 클립을 입력받고, 이미 재생중인 소리가 있어도 정지하지 않고 재생할 오디오와 중첩하여 재생
    ...
    // 재장전 시도
    public bool Reload() {
        // 재장전 실행에 성공하면 true, 재장전을 할 수 없는 상태면 false 반환
